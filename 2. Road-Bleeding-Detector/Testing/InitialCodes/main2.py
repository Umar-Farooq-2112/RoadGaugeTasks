import cv2
import numpy as np 
from basic_functions import binary_mask,filter_color,resize_image,join_images_horizontally,apply_threshold,get_largest_contours
from skimage import measure


def detectRoadBleed(image,mask,pothole,threshold = 15,dark_spots_threshold = 80,name = ''):
    ...
    _,pothole_filter = cv2.threshold(pothole_filter,int(np.mean(pothole_filter)),255,cv2.THRESH_BINARY)
...
#     print(f"{names[i]}:    {detectRoadBleed(image,mask,pothole,20,name = f"Results/{i}")}")
...
pothole = np.zeros_like(image)
detectRoadBleed(image,mask,pothole,20,name = "Tp")
